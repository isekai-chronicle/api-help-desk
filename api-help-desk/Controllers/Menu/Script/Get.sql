...
SELECT 	
	...
	UO.componentObject_id AS componentObject_id_user
FROM [security].menu As MM
	...
	LEFT JOIN frontend.componentObject AS COB ON COB.component_id = CO.component_id AND SUBSTRING(COB.[name], 1, 3) IN ('btn')
	LEFT JOIN [security].user_component AS UC ON UC.[user_id] = @user_id AND UC.component_id = CO.component_id
	LEFT JOIN [security].user_componentObject AS UO ON UO.[user_id] = @user_id AND UO.componentObject_id = COB.id 
